...
На основе анализа схемы и описания системы для включения проверочный лист были выбраны все пункты шаблона проверочного листа:
- Управление доступом - база любой системы безопасности, в которой уже в описании виден ряд проблем
- Безопасность данных - система работает с чуствительными данными, в том числе с персональными данными клиентов и собственников. При этом в описании команды практически не описан этот аспект, но описанные проблемы (нарушение контроля данных), позволяет предположить, что с этим всё плохо
- Защита инфраструктуры - у компании имеется приватное облако и собственный ЦОД, большинство IT-продуктов развёрнуто на собственных серверах, из чего следует, что компании следует озаботиться защитой и этого аспекта безопасности.
- Управление инцидентами - про систему управления инцидентами в описании ничего не сказано, по косвенным данным с этим тоже не очень хорошо: известны инциенты с доступом данных к другим ЖК, могут отображаться данные другого пользователя в личном кабинете, но источники проблемы неизвестны
- Специфичные аспекты: общие вопросы - из описания неясно, ограничен ли доступ к корпоративным ресурсам через VPN и мобильные устройства, поэтому этому вопросу тоже надо уделить внимание
- Управление оборудованием и физической безопасностью - компания крупная, нельзя пускать эти процессы на самотёк. Возможно эти процессы уже налажены, но в описании про это не сказано

## Проверочный лист
* если в описании нет явного упоминания, то ответ "нет" с комментарием "Нет данных"
...
| 3   | Проводится ли аудит и пересмотр учётных записей с повышенными привилегиями на регулярной основе?                                         |  нет   | готовится, судя по описанию, ранее не было.            |
| 4   | Реализован ли принцип минимальных привилегий при назначении ролей и прав доступа?                                                        |  нет   |  Нарушен контроль данных            |
| 5   | Автоматически ли удаляются или деактивируются учётные записи после увольнения или изменения должности сотрудников?                       |  нет   |  в описании не указано, но вероятно нет (см. Нарушен контроль данных)            |
...
| 9   | Созданы ли замкнутые контуры для подсистем, работающих с чувствительными данными?                                                        |  да    | для бухгалтерии есть             |
| 10  | Ограничен ли доступ к системам из внешних источников?                                                                                    |  да     |  Firewall для доступа клиентов и собственников            |
| 11  | Ведётся ли единый реестр учётных записей?                                                                                               |   нет  |   несколько точек регистрации нового клиент без координации            |
...
| 14  | Используются ли средства централизованного контроля доступа?                                                                            |   нет    |   API упавляющих компаний практически не контролируются           |
...
| 4   | Проводится ли регулярное резервное копирование данных? Хранятся ли резервные копии в защищённых хранилищах?                             |  да     |  резервное копирование для всех баз данных            |
...
| 2   | Реализована ли сегментация сети на логические зоны (DMZ, корпоративная сеть, гостевая сеть) для ограничения доступа к критическим ресурсам? |  да     |  для внешних пользователей            |
...
| 11  | Осуществляется ли фильтрация трафика между сетевыми сегментами?                                                                          | нет    |  нет на диаграмме            |
...
| 6   | Как часто проводятся внутренние и внешние аудиты безопасности? Обновляются ли политики и процедуры на основе результатов этих аудитов?  |   нет    | Только предстоит пройти, ранее, судя по всему, не было             |
